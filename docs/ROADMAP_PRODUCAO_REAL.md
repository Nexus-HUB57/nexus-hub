...
1.  **Remoção Urgente de Credenciais Hardcoded:**
    *   Remover imediatamente **TODAS** as credenciais hardcoded, incluindo a passphrase (`nexus-sovereign-passphrase-2026`), o salt (`nexus-salt`) do script `integrate_nexus_vault.ts` [2], e as `BINANCE_API_KEY`, `BINANCE_SECRET`, `ELECTRUM_USER`, `ELECTRUM_PASS` de `docs/real_production/main.py` [7].
...
### Fase 2: Infraestrutura e Integração **Real** com Mainnet
...
1.  **Nós de Blockchain Dedicados e Integração Direta:**
    ...
    *   **Substituir as simulações da classe `NexusRPC` em `docs/real_production/main.py` [7] por chamadas reais a esses nós ou APIs de provedores de infraestrutura.**
...
3.  **Verificação de Saldo e Transações em Tempo Real e Auditável:**
    ...
    *   **Garantir que todas as transações financeiras sejam executadas e verificadas na Mainnet, e não simuladas.**
